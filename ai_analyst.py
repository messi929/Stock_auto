...
{{
    ...
    "weekly_recap_kr": f"""당신은 삼성증권 리서치센터장 겸 한국투자증권 CIO입니다. 이번 한 주간의 한국 시장을 종합적으로 리뷰하는 **프리미엄 주간 리포트**를 작성하세요.
이 리포트는 주말에 발행되는 한 주의 총정리로, 평일 일간 리포트보다 훨씬 깊고 상세해야 합니다.
...
- 요일별로 "무슨 이유로 상승/하락했는지" 흐름을 내러티브로 서술 (월요일 A로 시작 → 화요일 B 반전 → ...)
- 거래대금/거래량의 주간 추이와 투자심리 판단

### 종목 심층분석
- 제공된 한국 10개 대표종목 각각에 대해 주간 성과를 개별 분석
- 각 종목별: 주간 등락률, 주간 내 고점/저점, 상승/하락 배경, 밸류에이션(PER/PBR) 수준, 동종업계 비교
- 종목을 섹터별로 그룹핑: 반도체(삼성전자/SK하이닉스), 플랫폼(NAVER/카카오), 자동차(현대차), 바이오(셀트리온), 2차전지(삼성SDI/포스코퓨처엠), 금융(신한지주/KB금융)
- 각 섹터 그룹의 주간 테마와 다음 주 전망

### 수급 분석
- 한 주간 외국인/기관/개인 누적 수급 흐름과 방향성 추론
- 업종별 수급 차별화: 어떤 업종에 외국인이 몰렸는지, 기관이 빠졌는지

### 글로벌 연동
- 환율(원/달러) 주간 흐름이 수출주/내수주에 미친 차별적 영향
- 글로벌 시장(미국/중국/유럽) 주간 동향이 한국 시장에 미치는 영향 연결
- 미국 Mag7 주간 성과가 한국 관련주(반도체/IT)에 미친 영향

### 차주 투자전략
- 다음 주 핵심 이벤트 캘린더(FOMC, 옵션만기, 실적발표 등)와 예상 영향
- 구체적인 섹터별 over/underweight 비중(%)과 근거
- 개별 종목 단위의 매수/매도/관망 의견과 목표가/손절가
- 시나리오별(베이스/불/베어) 구체적 대응 전략
...
{{
    ...
    "executive_summary": ["이번 주 가장 중요한 시장 변화 (주간 KOSPI/KOSDAQ 등락률 필수 포함)", "주간 수급/섹터 핵심 트렌드와 종목별 하이라이트", "다음 주를 위한 핵심 시사점과 전략 방향"],
    "market_overview": ["주간 KOSPI/KOSDAQ 종합 — 월요일부터 금요일까지 요일별 흐름을 내러티브로 서술. 각 요일의 시가→종가 변화, 장중 변곡점, 그 원인(뉴스/수급/외부변수)을 스토리텔링으로 연결. 주간 고저점과 거래대금 추이 (8~10문장)", "주간 수급 종합 — 외국인/기관/개인 5일간 누적 매매 동향, 업종별 수급 차별화, 프로그램 매매 영향, 주간 수급 반전/지속 판단. 외국인 선물/현물 연계 분석 (6~8문장)", "환율/원자재/글로벌 연동 — 원/달러 주간 흐름과 배경, WTI/금/BTC 변동, 미국 3대 지수 주간 성과가 한국 시장에 미친 영향 경로 분석. 달러인덱스/미국채 금리 변화와 원화 가치 연동 (6~8문장)", "글로벌 매크로 환경 — 미국/중국/유럽 주간 핵심 이벤트와 한국 시장 영향, 연준 정책 기대 변화, 글로벌 유동성 환경 (5~6문장)"],
    "sector_analysis": ["반도체 섹터 심층 — 삼성전자·SK하이닉스 주간 개별 분석: 주간 등락률, 거래량, 외국인 수급, 글로벌 반도체 사이클 위치, TSMC/엔비디아 연동, 다음 주 전망. PER/PBR 현재 수준과 밴드 내 위치 (8~10문장)", "플랫폼/IT — NAVER·카카오 주간 개별 분석: 사업부문별 모멘텀(광고/커머스/AI/핀테크), 주간 수급, 밸류에이션 비교, 경쟁사 대비 포지션 (6~8문장)", "자동차/바이오/2차전지 — 현대차·셀트리온·삼성SDI·포스코퓨처엠 주간 분석: 각 종목 등락 배경, 업황 사이클, 글로벌 peer 비교, 다음 주 촉매 (8~10문장)", "금융 섹터 — 신한지주·KB금융 주간 분석: 금리 환경과 NIM 전망, 배당 매력도, 외국인 수급 패턴, 밸류에이션 (5~6문장)", "주간 테마 종합 — AI/HBM/온디바이스AI/로봇/방산/원전 등 테마별 주간 흐름, 대형주 vs 중소형주 스타일 분석, KOSPI200 vs KOSDAQ150 차별화 (5~6문장)"],
    "technical_levels": ["KOSPI 주간 기술적 심층 — 주봉 기준 5주/20주/60주 이동평균선 위치와 배열(정배열/역배열), 주간 캔들 패턴(양봉/음봉/십자/도지), 볼린저밴드 위치, RSI/MACD 주간 시그널, 거래량 동반 여부, 다음 주 핵심 지지선·저항선 구체적 수치와 근거 (6~8문장)", "KOSDAQ 주간 기술적 분석 — 동일 구조로 상세 분석 (5~6문장)", "원/달러 주간 기술적 분석 — 주간 추세, 주요 레벨, 다음 주 예상 밴드 (4~5문장)"],
    "money_flow": ["외국인 주간 누적 매매 심층 — 순매수/순매도 금액, 업종별 패턴(반도체/금융/자동차 등), 선물 포지션 변화, 프로그램 매매 비중, MSCI 리밸런싱 영향, 향후 외국인 방향성 전망 (6~8문장)", "기관 주간 매매 — 연기금/투신/보험/사모 각 주체별 주간 행태와 의미, 업종별 기관 선호도 변화 (5~6문장)", "개인/신용잔고/공매도 — 개인 투자자 행태, 신용잔고 변화와 레버리지 수준, 공매도 잔고 변화와 숏커버 가능성, 전체 투자심리 진단 (4~5문장)"],
    "key_numbers": ["이번 주 핵심 숫자 1: 수치 + 주간 맥락에서의 의미 해석", "핵심 숫자 2", "핵심 숫자 3", "핵심 숫자 4", "핵심 숫자 5"],
    "risk_factors": ["다음 주 핵심 리스크 1 — 발생 시나리오, 임계점(구체적 수치), KOSPI 영향 범위, 해당 리스크가 현실화될 때 타격받을 종목/섹터 명시 (5~6문장)", "리스크 2 — 동일 구조 (5~6문장)", "테일 리스크 — 낮은 확률이지만 발생 시 고영향 이벤트. 발생 시 시장 영향 시뮬레이션 (4~5문장)"],
    "strategy": ["다음 주 공격적 전략 — 주목 섹터/종목 구체적 명시(삼성전자/SK하이닉스/NAVER 등 실제 종목명), 진입 조건(기술적 레벨), 목표가, 손절 기준, 비중 제안(%) (6~8문장)", "다음 주 균형 전략 — 포트폴리오 비중 조절: 주식/채권/현금 비중(%), 업종별 over/underweight(반도체 +5%, 바이오 -3% 등 구체적 수치), ETF 활용 방안 (5~7문장)", "다음 주 방어적 전략 — 리스크 헤지 구체적 방법(인버스 ETF 티커, 풋옵션 행사가, 현금 비중 목표%), 방어주(통신/유틸리티/고배당) 편입 근거 (4~6문장)", "핵심 트레이드 아이디어 — 이번 주 데이터를 종합했을 때 확신도가 가장 높은 1~2가지 투자 아이디어. 구체적 종목/ETF, 진입가/목표가/손절가, 기대 수익률 명시 (4~5문장)"],
    "tomorrow_outlook": ["다음 주 베이스 시나리오(확률 55%) — KOSPI 예상 밴드(구체적 포인트), 요일별 예상 흐름, 핵심 촉매(catalyst) (5~6문장)", "불 시나리오(확률 25%) — 상방 트리거 구체적 조건, 돌파 시 KOSPI 목표, 수혜 종목 (4~5문장)", "베어 시나리오(확률 20%) — 하방 트리거, 지지선 이탈 시 하방 목표, 방어 종목, 손절 기준 (4~5문장)", "다음 주 핵심 이벤트 캘린더 — 월/화/수/목/금 날짜별 주요 일정(경제지표 발표, 실적 발표, FOMC, 옵션만기 등)과 각 이벤트의 예상 시장 영향을 구체적으로 서술 (5~6문장)"]
}}""",

    "weekly_recap_us": f"""당신은 JP모건 수석 글로벌 전략가 겸 골드만삭스 CIO입니다. 이번 한 주간의 미국 시장을 종합적으로 리뷰하는 **프리미엄 주간 리포트**를 작성하세요.
이 리포트는 주말에 발행되는 한 주의 총정리로, 평일 일간 리포트보다 훨씬 깊고 상세해야 합니다.
...
- 요일별로 "무슨 이유로 상승/하락했는지" 흐름을 내러티브로 서술

### Mag7 종목 심층분석
- 제공된 Mag7 7개 종목 각각에 대해 주간 성과를 개별 심층 분석
- 각 종목별: 주간 등락률, 주간 내 고점/저점, 상승/하락 배경
- 사업부문별 분해: 클라우드(MSFT/AMZN/GOOGL), AI(NVDA/MSFT/GOOGL), 광고(META/GOOGL), 하드웨어(AAPL), EV/에너지(TSLA)
- 밸류에이션(Forward PER) 현재 수준과 역사적 밴드 내 위치
- 각 종목의 다음 주 촉매와 전망
...
### 차주 투자전략
- 한국 투자자 관점에서의 시사점과 실행 가능한 전략 (환헤지 포함)
- 다음 주 핵심 이벤트(FOMC, 고용지표, 실적발표 등)와 예상 영향
- 구체적 종목/ETF 단위의 매수/매도 의견과 목표가
...
{{
    ...
    "executive_summary": ["이번 주 가장 중요한 시장 변화 (주간 3대 지수 등락률 필수 포함)", "주간 Mag7 개별 성과 하이라이트와 섹터 핵심 트렌드", "다음 주를 위한 핵심 시사점과 전략 방향"],
    "market_overview": ["3대 지수 주간 종합 — 월요일부터 금요일까지 요일별 흐름을 내러티브로 서술. 각 요일의 상승/하락 원인, 장중 변곡점, 지수별 차별화 이유를 스토리텔링으로 연결. 거래량/market breadth 변화 (8~10문장)", "연준/금리/달러 주간 동향 — Fed Fund Futures가 반영하는 금리 기대 변화, 2년/10년물 스프레드, 달러인덱스 흐름, 실질금리 변화, 유동성 환경 변화 (6~8문장)", "매크로 지표 주간 종합 — 이번 주 발표된 모든 경제지표(고용/소비/제조업/인플레이션/주택)와 각 지표에 대한 시장 반응. 지표 서프라이즈 vs 기대치 분석 (6~8문장)", "글로벌 연동 — 유럽/중국/일본 시장 주간 동향, 원자재(WTI/금/구리) 흐름, 신흥국 자금 유출입, 글로벌 리스크 요인 (5~6문장)"],
    "sector_analysis": ["NVIDIA 심층 — 주간 등락률, 일별 흐름, AI 칩 수요/공급 사이클, 데이터센터 capex 트렌드, 밸류에이션(Forward PER), 주간 거래량, 기관 수급, 다음 주 촉매, 한국 반도체주 연동 영향 (6~8문장)", "Apple·Microsoft 심층 — 각 종목 주간 성과, 사업부문별 모멘텀(iPhone/Mac/서비스/클라우드/AI Copilot), 밸류에이션 비교, 주간 수급, 다음 주 전망 (6~8문장)", "Alphabet·Meta·Amazon 심층 — 각 종목 주간 성과, 광고 시장 동향, 클라우드 성장률, AI 투자 ROI, 밸류에이션, 규제 리스크, 다음 주 전망 (6~8문장)", "Tesla 심층 — 주간 성과, EV 시장 점유율 동향, 에너지사업/FSD/로봇 진행 상황, 밸류에이션(전통 자동차 대비), 변동성 분석 (5~6문장)", "11개 GICS 섹터 주간 성과 순위 — 상위/하위 3개 섹터, 자금 이동 방향, 섹터 로테이션 의미, 주목 ETF(SPY/QQQ/XLK/XLF 등 구체적 티커) (5~6문장)"],
    "technical_levels": ["S&P 500 주간 기술적 심층 — 주봉 기준 5주/20주/60주 이동평균선 배열, 주간 캔들 패턴, 볼린저밴드, RSI/MACD 주간 시그널, 피보나치 레벨, 다음 주 핵심 지지·저항선(구체적 수치) (6~8문장)", "NASDAQ 주간 기술적 분석 — 동일 구조로 상세 분석 (5~6문장)", "다우존스/러셀2000/VIX 기술적 분석 — 대형주 vs 소형주 괴리, VIX 구조(콘탱고/백워데이션), 공포/탐욕 지표 (4~5문장)"],
    "money_flow": ["기관 주간 자금흐름 — ETF 유입/유출 순위(구체적 ETF명과 금액), 옵션 시장 포지셔닝 변화, Put/Call 비율 추이, 감마 노출 분석, 기관 레버리지 수준 (6~8문장)", "해외 자금흐름 — 신흥국 vs 선진국 자금 이동, 달러 강세/약세와 연동, 글로벌 펀드 플로우, 한국/일본/대만 외국인 자금 비교 (5~6문장)"],
    "key_numbers": ["이번 주 핵심 숫자 1: 수치 + 주간 맥락에서의 의미 해석", "핵심 숫자 2", "핵심 숫자 3", "핵심 숫자 4", "핵심 숫자 5"],
    "risk_factors": ["다음 주 지정학/정책 리스크 — 구체적 시나리오, 발생 확률, 시장 영향 경로, 타격 종목/섹터 명시 (5~6문장)", "금리/인플레 리스크 — 연준 경로 변경 트리거, 금리 급등 시나리오, 영향받을 섹터/종목 (5~6문장)", "시스템/유동성 리스크 — 레버리지 수준, 신용 스프레드, 시장 구조 취약점, 역사적 유사 사례 비교 (4~5문장)"],
    "strategy": ["다음 주 미국 직접투자 전략 — 섹터별 over/underweight(구체적 %와 근거), 매수 추천 종목/ETF(티커·진입가·목표가·손절가), 매도/관망 종목 (6~8문장)", "한국 투자자 관점 — 원/달러 환헤지 전략(환헤지 비중 %), ADR 활용법, 미국 시장 연동 한국 종목(반도체·2차전지 등), 환율 시나리오별 대응 (5~7문장)", "리스크 관리 — 포지션 사이징(총 자산 대비 %), VIX 활용 헤지(VIX ETF 티커), 현금 비중 목표, 스톱로스 전략, 이익 실현 기준 (5~6문장)", "핵심 트레이드 아이디어 — 이번 주 데이터를 종합했을 때 확신도가 가장 높은 1~2가지 투자 아이디어. 구체적 종목/ETF, 진입가/목표가/손절가, 기대 수익률, 시간 프레임 명시 (4~5문장)"],
    "tomorrow_outlook": ["다음 주 핵심 이벤트 캘린더 — 월/화/수/목/금 날짜별 주요 일정(FOMC/고용/실적/CPI/PPI/소매판매 등)과 각 이벤트의 시장 예상 반응을 구체적으로 서술 (5~6문장)", "다음 주 시나리오별 전망 — 베이스(55%): S&P500/NASDAQ 예상 밴드, 불(25%): 상방 트리거와 목표, 베어(20%): 하방 트리거와 지지선 (5~6문장)", "핵심 모니터링 포인트 — 금리(10Y/2Y 스프레드), 달러인덱스, VIX, 옵션 만기, 기관 포지셔닝 (4~5문장)"]
}}""",
}
...
def generate_analysis(data_summary, report_type="post_market"):
    ...
    # === 1차 호출: 전체 분석 생성 ===
    # 주간 리포트는 더 상세한 분석을 위해 토큰 상향
    is_weekly = report_type in ("weekly_recap_kr", "weekly_recap_us")
    max_tokens = 16000 if is_weekly else 12000

    weekly_instruction = ""
    if is_weekly:
        weekly_instruction = " 이 리포트는 주말에 발행되는 프리미엄 주간 리포트입니다. 평일 일간 리포트보다 2~3배 더 상세하고 깊이 있게 작성하세요. 특히 개별 종목 심층분석, 요일별 내러티브, 차주 구체적 투자전략에 역량을 집중하세요."

    payload = {
        "model": MODEL,
        "max_tokens": max_tokens,
        "messages": [
            {"role": "user", "content": f"## 시장 데이터\n\n{data_summary}\n\n위 데이터를 분석하여 JSON으로 응답하세요. 반드시 모든 섹션(executive_summary, market_overview, sector_analysis, technical_levels, money_flow, key_numbers, risk_factors, strategy, tomorrow_outlook)을 빠짐없이 작성하세요. 각 섹션을 최대한 풍부하고 깊이 있게 작성하세요.{weekly_instruction}"}
        ],
