...
├── publish_log.jsonl         # 발행 이력 로그 (자동 생성, .gitignore)
├── .github/
│   └── workflows/
...
|------|------------|-------------------|---------------|
| `pre_market` | 골드만삭스/모건스탠리 수석 애널리스트 | 매일 07:57 | 2 |
...
- max_tokens: 4,000 / timeout: 180초
- **안정성**: 3회 재시도 + 5초 딜레이, 전체 실패 시 빈 분석 반환
...
| Claude API 타임아웃/오류 | 3회 재시도 (5초 간격), 전체 실패 시 기본 분석으로 대체 |
| AI 응답 JSON 파싱 실패 | `{`~`}` 추출, 코드블록 제거 후 재시도, 최종 실패 시 빈 분석 |
| AI 분석 모듈 예외 | try/except로 파이프라인 계속 진행 |
...
| us_market TimeoutError | API 응답 지연 (분석 데이터 많음) | timeout 180초 + 3회 재시도 (이미 적용) |
